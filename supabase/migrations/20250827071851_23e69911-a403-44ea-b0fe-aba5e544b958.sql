...
-- Create validation function for family member limit (replacing check constraint)
...
BEGIN
  IF (SELECT COUNT(*) FROM public.family_members 
    ...
    RAISE EXCEPTION 'Maximum of 5 active family members allowed per account';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER validate_family_member_limit_trigger
